...
export default function LanguageList() {
    ...
    const [languages, setLanguages] = useState<RPGSheetGURPSLanguages[]>([]);
    const [newLanguage, setNewLanguage] = useState<RPGSheetGURPSLanguages>({
        name: '',
        falada: '',
        escrita: '',
        pontos: ''
    ...
    const [editingLanguage, setEditingLanguage] = useState<RPGSheetGURPSLanguages>({
        name: '',
        falada: '',
        escrita: '',
        pontos: ''
    ...
    return (
        <>
            {languages.length > 0 && (
                <div className="mb-4">
                    <div className="grid grid-cols-6 font-bold mb-1">
                        <div>Nome</div>
                        <div>Falada</div>
                        <div>Escrita</div>
                        <div>Pontos</div>
                        ...
                        <div key={index} className="grid grid-cols-6 mb-1 text-center items-center">
                            {editingIndex === index ? (
                                <>
                                    ...
                                    <input
                                        className="border border-gray-300 w-full text-center p-1"
                                        value={editingLanguage.falada}
                                        onChange={(e) => handleEditChange('falada', e.target.value)}
                                    />
                                    <input
                                        className="border border-gray-300 w-full text-center p-1"
                                        value={editingLanguage.escrita}
